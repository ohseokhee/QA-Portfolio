...
class TestE2EPOM:
    ...
    def test_bing_search_firefox(self, firefox_driver, db_conn):
        """Bing 검색 → DB 저장 (Firefox)"""
        page = BingPage(firefox_driver)
        page.open()
        page.search(SEARCH_KEYWORD)
        title = page.get_result_title()
        ...
        with db_conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO search_results (browser, site, keyword, result_title) "
                "VALUES (%s, %s, %s, %s)",
                ("firefox", "bing", SEARCH_KEYWORD, title),
            )
        db_conn.commit()
        print(f"[Firefox][Bing] 저장 완료: {title}")
    ...
    def test_db_verify(self, db_conn):
        ...
        assert count >= 2, f"DB 저장 건수 부족: {count}"
