...
class TestE2ERDS:
    ...
    def test_bing_search_chrome(self, chrome_driver, db_conn):
        ...
        db_conn.commit()
        print(f"[Chrome][Bing→RDS] 저장 완료: {title}")

    def test_rds_verify(self, db_conn):
        """RDS 저장 결과 검증"""
        with db_conn.cursor() as cursor:
            cursor.execute(
        ...
        assert count >= 1, f"RDS 저장 건수 부족: {count}"
        print(f"[RDS 검증] 저장된 검색 결과 건수: {count}")
